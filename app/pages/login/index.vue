<template>
  ...
  >
    <div
      class="bg-white px-8 py-10 rounded-2xl shadow-2xl min-w-[320px] max-w-[90vw] flex flex-col items-stretch"
    >
      <h2
        class="text-center mb-6 text-gray-800 font-bold tracking-widest text-2xl"
      >
        {{ isLogin ? "登入" : "註冊" }}
      </h2>
...
async function handleSubmit() {
  ...
  // axios / $fetch
  try {
    await $fetch("http://localhost:5042/api/auth/login", {
      method: "POST",
      body: fd,
    });
